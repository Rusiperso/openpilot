...
class CarrotServ:
  ...
  def _update_tbt(self):
    ...
    # v: 재억 재지적(2026-09-10) - 138(S)을 정확히 180도로 두면, 아이콘의 진입 지점도
    # 항상 180도(6시 고정)라서 진출 지점과 완전히 겹쳐 화살표가 진입 꼬리에 묻혀 안
    # 보였음. "왼쪽 급"이 실제로 커버하는 범위(180~225도)에서 경계값 대신 중간값(202도)을
    # 써서 겹침 없이 의미도 그대로 유지. #문제시 원복
    rotary_code_to_angle = {
      131: 45, 133: 90, 134: 135, 138: 202,
